...
- **A blocking gate's enumerated remedies are now mechanically binding.** When a
  gate blocks with a list of ways forward, that list is the option set — and the
  agent relaying it is the weakest point. Measured: a review cap printed three
  remedies and the relay dropped the first, invented a fourth, and added the one
  outcome the cap exists to prevent. The gate's own text was one message back, so
  this was a strong prior overriding what was on screen rather than anything being
  forgotten — which is why re-showing the text is not the fix.

  The remedy set is now written to state as data rather than living only in prose.
  While a demand is unacknowledged, an `AskUserQuestion` is refused unless one of
  its questions offers every declared remedy, and the acknowledgment must name the
  choice (`# escalation-ack:redesign`, not a bare `# escalation-ack`). The block
  message prints the exact runnable form for each remedy, so the vocabulary is
  visible even when the state write failed — a bookkeeping failure never downgrades
  to accepting an unnamed ack. A demand records the session that owes it, so one
  session's obligation is never presented to — or discharged by — another.

  **The covering question's options must BE the remedy set** — one option per
  remedy, matched on the option's label, and nothing else in that question. Three
  counting rules were defeated before this one, each narrower than the last, and
  the reason is structural: the session writes the option text, so no count over
  those words closes the gap. "Each remedy matches some option" fell to a single
  option naming all three. "A distinct option per remedy" fell to the same option
  absorbing whichever remedy it was matched against. "Exactly one remedy named
  per option" closed both and still passed the acceptance case, because a
  presence test cannot see an EXTRA option — and an added "ship as-is" beside the
  three real remedies is the measured corruption almost verbatim. A bijection can
  see it. Descriptions no longer count toward a match either: prose about an
  option is not the thing the user picks, and matching it let "Ship as-is /
  neither shelve nor rework" register as offering `shelve`.

  **There is deliberately no "proceed as-is" escape.** Accepting the findings as
  they stand is what the terminal offers, several rounds further on; withholding
  it at this cap is what the cap is. An earlier revision added one, verified
  against the session transcript, and it was removed rather than fixed — the
  evidence could not support the claim, since the check never read which option
  the user chose, so a user who picked "redesign" still licensed it.

  Fail directions are set per layer and each is pinned by a test. The ask-time
  guard fails OPEN and is deliberately not wired through `run_guard`: a guard able
  to refuse a question could otherwise leave a session unable to ask anything at
  all, including how to unwedge itself. A malformed kill-switch config leaves it
  ARMED, since silently disarming a gate on a typo is the worse failure and the
  environment variable remains an unconditional escape. A payload whose shape has
  drifted stands the guard down rather than blocking every question in the session.

  Also here: the two readers of an override sigil now share one scanner instead of
  two copies of the same quote-walking loop, so "recognised by one, missed by the
  other" is impossible by construction; the new parser symbol is imported at call
  time rather than module scope, so version skew between a worktree and the main
  tree fails CLOSED under `run_guard` instead of widening the standing
  import-time fail-open; the round-counter file is written atomically, so a
  concurrent reader can no longer catch a truncated file and fail the whole gate
  open; and the counter's writers now carry forward a named set of fields instead
  of each re-listing the survivors, which is how the demand came to be dropped by
  one writer while another kept it.
